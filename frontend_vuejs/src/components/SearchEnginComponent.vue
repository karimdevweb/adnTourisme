<script>
    ...
    import { LMap, LTileLayer, LMarker, LPopup, LTooltip } from 'leaflet';
    import  axios from 'axios'
    import { isProxy, toRaw } from 'vue';
    import { mapGetters , mapMutations, mapActions } from 'vuex';

    export default{
        name:'SearchEnginComponent',
        data(){
            return{
                path: 'http://127.0.0.1:5000/',
                searched_dict: {categories : [],
                                regions:[],
                                query:''} ,

                // get the region by hover
                departement_mousover : '',
                
                // variable for displaying the div without query 
                no_searched_query: Boolean,


                // display the select all btn
                select_all_btn : Boolean,
        ...
        methods: {
            ...mapActions(['getOnePoiAction']),
            ...mapMutations(['changeSearchEnginVisibility_toFalse']),
            get_one_poi(){
                //  convert our proxy obk=ject into raw object
                let res = this.searched_dict
                if(isProxy(this.searched_dict)){
                    res = toRaw(this.searched_dict)
                }

                //  get  only the values as dict
                let res_dict =  {path : this.path , categories: res.categories, regions: res.regions ,query: res.query }
                this.getOnePoiAction( res_dict )


                // check if the query contains a name or a category
                if((!this.searched_dict.categories.length ) & (!this.searched_dict.query.length) & (!this.searched_dict.regions.length)){
                    this.no_searched_query = true
                }else{
                    this.no_searched_query = false
                }
            },
            //  get the button value and adding it into the dict
            addingBValue($event){
                const target = $event.target.value
                if (this.searched_dict.categories.includes(target)){
                    const index = this.searched_dict.categories.indexOf(target)
                    this.searched_dict.categories.splice(index, 1)

                }else
                {
                    this.searched_dict.categories.push(target)
                }
             ...
             get_region($event,value){
                const target = value
                if (this.searched_dict.regions.includes(target)){
                    const index = this.searched_dict.regions.indexOf(target)
                    this.searched_dict.regions.splice(index, 1)

                }else
                {
                    this.searched_dict.regions.push(target)
                }
            ...
            addingQuery($event){
                ...
                this.searched_dict.query= $event.target.value.trim()
            ...
            contains(item){
                ...
                if(this.searched_dict.categories.includes(item))
                {
                    return true
                }else
                {
            ...
            // check if the path is in the dict, then toggle color when adding it
            contains_region(item){
                if(this.searched_dict.regions.includes(item))
                {
                    return true
                }else
                {
            ...
            // condition for colors and icons 
            iconObject(item){
                let icon_dict = {housing : {icon:'bed' , bg_color: 'bg_red'},
                                local_business : {icon:'industry' , bg_color: 'bg_green'},
                                catering : {icon:'drumstick-bite' , bg_color: 'bg_blue'},
                                event : {icon:'cake-candles' , bg_color: 'bg_orange'},
                                cultural_sites : {icon:'building-columns' , bg_color: 'bg_yellow'},
                                tours : {icon:'route', bg_color: 'bg_white_bleu'},
                                sports_and_Leisure: {icon:'person-hiking' , bg_color: 'bg_pink'},
                                religions_sites: {icon:'church' , bg_color: 'bg_brown'},
                                tourist_information_center: {icon:'circle-info' , bg_color: 'bg_black'},
                                transportation: {icon:'bus' , bg_color: 'bg_rose'}}
                    let res = icon_dict[item]
                    let icon = res.icon
                    let bg_color = res.bg_color
                return [icon , bg_color]
            },

            // get only the clicked element and display graphs with its data
            getOnePoiMethod($event, value){
                
                let res_one_poi = value
                if(isProxy(value)){
                    res_one_poi = toRaw(value)
                }
                this.one_poi_variable= res_one_poi
               
            },
           
           

            // close the search engine and clear all the  output
             // changeSearchEnginVisibility_toFalse
            SearchEnginMethod($event){
                
                this.changeSearchEnginVisibility_toFalse({visibility : false})
            },
            onePOIMethodToFalse($event){
                
                this.one_poi_variable = {}
            },
            ...
            // -------------------------------------------------------------------------
             // -------------------------------------------------------------------------
            // map for only one  poi
            setupLeafletMap: function () {
                this.leaflet_one_poi_map= L.map("mapContainerOnePoi").setView([this.one_poi_variable.lat, this.one_poi_variable.long], this.zoom);
                L.tileLayer(
                    this.url,
                    {
                ...
                ).addTo(this.leaflet_one_poi_map);

                // create a new icon with custom color
                let greenIcon = L.icon({
                    iconUrl: '../../src/assets/marker_icon_green.png',
                    iconSize: [25, 41],
                    iconAnchor: [12, 41],
                    popupAnchor: [1, -34],
                });

                // display my main icon 
                let main_marker = L.marker([this.one_poi_variable.lat, this.one_poi_variable.long] , {icon: greenIcon});
                    main_marker.bindTooltip(`<div class='tooltipe_div'> établissement: <span class='span_tooltipe'>${this.one_poi_variable.name}</span>, <br> contactez au : 
                                                <span class='span_tooltipe'>${this.one_poi_variable.contact}</span> 
                                                <br> categorie: <span class='span_tooltipe'>${this.one_poi_variable.category}</span> </div>`).openTooltip();
                    main_marker.addTo(this.leaflet_one_poi_map);
            ...
            markersFuncLoop(){
                ...
                this.getListProp.data.forEach((item,index)=>{
                    
                    if((item.commune == this.one_poi_variable.commune )  & (item.commune != 'non renseignée') & (item.name != this.one_poi_variable.name ))
                    {   
                        ...
                        marker.addTo(this.leaflet_one_poi_map)
                    }
                })
        ...
        watch: {
            // Watch for changes to the one_poi_variable data property
            one_poi_variable : function(newVal, oldVal)
            {   
                if(newVal && newVal.name && !this.mapContainer)
                {
                    // Delay the loading of the map until the mapContainerOnePoi element is available
                    this.$nextTick( () =>
                    {
                        const mapContainer = document.getElementById('mapContainerOnePoi');
                        if(mapContainer)
                        {
...
<template>
    <section class="searchbar_section" id="searchbar_section">
        ...
        <div class="result_display_div">
            <div class="no_serched_query_div" v-if="this.no_searched_query">
               
                <fa class="no_result_icon" icon="list-check"/>
                <p>we display the result here</p>
            </div>
            <div class="searched_query_div" v-else>
                <div class="no_serched_query_div" v-if="(!getListProp.data.length) || ( getListProp.data == undefined )">
                   
                    <fa class="no_result_icon" icon="kiwi-bird"/>
                    <p>please, be aware of what you submited</p>
                </div>
                <div class="one_result_display" v-for="(value, index) in getListProp.data" :key="index"  @click="getOnePoiMethod($event,value)">
                    <p class="result_sentence"><fa class="icon_result_search_bar" :class="iconObject(value.category)[1]" :icon="iconObject(value.category)[0]"/>{{value.name}} - {{ value.postal_code }} - {{ value.region }}</p>
                </div>
            </div>
        ...
        <!-- one poi result section  -->
        <Transition name="slide-fade">
            <section class="one_poi_result_section" v-if="this.one_poi_variable.name">
                <div class="mixer_div" @click="onePOIMethodToFalse">
                    <fa class='trash_icon' icon="trash-can"/>
                </div>

                <div class="description_one_poi_div">    

                    <div class="cards_description_div">
                        <div class="name_div">
                            <fa class="icon_result_search_bar" :class="iconObject(this.one_poi_variable.category)[1]" :icon="iconObject(this.one_poi_variable.category)[0]"/>
                            {{  this.one_poi_variable.name }}
                        </div>


                        <div class="one_description_div" :title="'region'" :class="{'no_reseigned_data':this.one_poi_variable.region == 'non renseignée'}">
                            <fa class="icon_description" icon="map-location-dot" :class="{'no_reseigned_data':this.one_poi_variable.region == 'non renseignée'}"/>
                            {{  this.one_poi_variable.region }}
                        </div>
                        <div class="one_description_div"  :title="'departement'" :class="{'no_reseigned_data':this.one_poi_variable.departement == 'non renseignée'}">
                            <fa class="icon_description" icon="map-location" :class="{'no_reseigned_data':this.one_poi_variable.departement == 'non renseignée'}"/>
                            {{  this.one_poi_variable.departement }}
                        </div>
                        <div class="one_description_div" :title="'commune'" :class="{'no_reseigned_data':this.one_poi_variable.commune == 'non renseignée'}">
                            <fa class="icon_description" icon="map-pin" :class="{'no_reseigned_data':this.one_poi_variable.commune == 'non renseignée'}" />
                            {{  this.one_poi_variable.commune }}
                        </div>
                        <div class="one_description_div" :title="'category'" :class="{'no_reseigned_data':this.one_poi_variable.category == 'non renseignée'}">
                            <fa class="icon_description" icon="cat" :class="{'no_reseigned_data':this.one_poi_variable.category == 'non renseignée'}" />
                            {{  this.one_poi_variable.category }}
                        </div>
                        <div class="one_description_div" :title="'contact'" :class="{'no_reseigned_data':this.one_poi_variable.contact == 'non renseignée'}">
                            <fa class="icon_description" icon="phone-volume" :class="{'no_reseigned_data':this.one_poi_variable.contact == 'non renseignée'}"/>
                            {{  this.one_poi_variable.contact }}
                        </div>
                        

                    </div>
                    <div class="desription_div">
                        <span>{{  this.one_poi_variable.description_x}}</span>
                    </div>
                    <div class='identity_main_div'>
                        <div class="identity_div">
                            <div class="span_title_div">
                                <span class="span_title"> lastUpdated:</span>
                                <span class="span_title"> identifier:</span>
                                <span class="span_title"> latitude:</span>
                                <span class="span_title"> longitude:</span>
                                <span class="span_title"> num département:</span>
                                <span class="span_title"> adresse postale:</span>
                                <span class="span_title"> diffuseur:</span>
                                <span class="span_title"> publiié par:</span>
                            </div>
                            <div class="explication_span_title">
                                <span :class="{'no_reseigned_data':this.one_poi_variable.lastUpdate == 'non renseignée'}">{{  this.one_poi_variable.lastUpdate }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.identifier == 'non renseignée'}">{{  this.one_poi_variable.identifier }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.lat == 'non renseignée'}">{{  this.one_poi_variable.lat }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.long == 'non renseignée'}">{{  this.one_poi_variable.long }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.num_dept == 'non renseignée'}">{{  this.one_poi_variable.num_dept }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.adresse_postale == 'non renseignée'}">{{  this.one_poi_variable.adresse_postale + ' ' +this.one_poi_variable.postal_code}}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.sit_diffuseur == 'non renseignée'}">{{  this.one_poi_variable.sit_diffuseur }}</span>
                                <span :class="{'no_reseigned_data':this.one_poi_variable.published_by == 'non renseignée'}">{{  this.one_poi_variable.published_by }}</span>
                            </div>
                        </div>   
                        <div class="google_api_tendance">

                            <!-- displaying my map -->
                            <div class="mapContainerOnePoi" id="mapContainerOnePoi">
                            </div>
...
<style setup>
    ...
    .one_poi_result_section{
        ...
        padding: 10px;
        border-radius: 10px;
        box-shadow: 3px 3px 20px 1px rgba(0, 0, 255, .2);
        font-size: 18px;
        line-height: 30px;
    ...
    .form{
        width: 50%;
        height: 80%;
        border: 1px dashed rgba(0, 0, 255, .2);
        border-radius: 10px;
        display: flex;
        flex-direction: column;
    ...
    .search_input_icon_div>span:after{
        content: '';
        margin: 10px;
        width: 1px;
        height: 100%;
    ...
    .search_engin_input:focus{
        border: unset;
        outline: none;


    }
    .search_engin_category_div{
        width: 100%;
        height: 65%;
        padding: 5px;
        display: flex;
    ...
    .departement_hover_effect_ul{
        height: 25%;
        width: 100%;
        margin: 10px 0;
        list-style: none;
    }
    .svg_departement_list_ul{
        padding: 10px;
        margin: 0;
        list-style: none;
          
    }
    .departement_list_div_overflow{
        width: 100%;
        height: 75%;
        overflow: auto;
        margin: 10px 0;
    ...
    .one_result_display{
        ...
        width: 100%;
        border-bottom: 0.2px solid rgba(0, 0, 255, .2);
    ...
    .searched_query_div{
        ...
        align-items: center;
        overflow: auto;
        border-bottom: 2px solid rgba(73, 70, 70, 0.493);
    }
    .no_serched_query_div{
        height: 50%;
        width: 50%;
        display: flex;
        flex-direction: column;
